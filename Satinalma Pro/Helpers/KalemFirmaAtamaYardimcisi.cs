...
public static class KalemFirmaAtamaYardimcisi
{
    ...
    public static string OzetMetni(SatinalmaTalepKalemi kalem, IEnumerable<SatinalmaTeklif> teklifler)
    {
    ...
    public static void FirmaMiktariniAyarla(SatinalmaTalepKalemi kalem, Guid teklifId, double miktar)
    {
        ...
        if (diger.Count == 0)
        {
            if (Math.Abs(miktar - kalem.Miktar) > Tolerans)
                throw new InvalidOperationException(
                    "Tek firmada miktar talep toplamına eşit olmalı. Başka firma ekleyin (Ctrl+tık) veya tüm miktarı seçin.");
