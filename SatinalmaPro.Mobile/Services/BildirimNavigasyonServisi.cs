...
public static class BildirimNavigasyonServisi
{
    ...
    public static async Task FcmVeridenGitAsync(IDictionary<string, string> veri)
    {
        ...
        if (veri.TryGetValue("deepLink", out var deepLink) && !string.IsNullOrWhiteSpace(deepLink) &&
            DeepLinkServisi.Coz(deepLink) is { } dl)
        {
            await RouteGitAsync(DeepLinkServisi.MobilRoute(dl), oturum);
            return;
        }
